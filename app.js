...
function bindAllButtonsOnclick() {
  ...
  const mappings = {
    ...
    "welcome-btn-signup": () => showAuthScreen("signup"),
    "welcome-btn-guest": async () => {
      try {
        await FirebaseMock.auth.signInAsGuest();
      } catch(e) {
        alert("Guest login failed");
    ...
    // Sign In screen
    "signin-submit-btn": () => handleSignIn(),
    "signin-btn-otp-mode": () => {
      alert("Phone OTP Sign-In is currently simulated. Please sign in using any email and password, or use Google/Apple Sign-In.");
    },
    "signin-back": () => showAuthScreen("welcome"),
    "signin-google": async () => {
      try {
        await FirebaseMock.auth.signInWithGoogle();
        showNotification("Google Login Success", "Logged in via Google verified profile!");
    ...
    // Sign Up screen
    "signup-submit-btn": () => handleSignUp(),
    "signup-back": () => showAuthScreen("welcome"),
    // OTP verify
    ...
    "forgot-reset-btn": () => handlePasswordReset(),
    // Header actions
    ...
    "btn-save-doc": () => triggerSaveDocument(),
    "btn-share-doc": () => triggerShareLink(),
    "btn-summarize-doc": () => triggerSummaryReader(),
    "btn-util-rewrite": () => triggerAssistAction("rewrite"),
    "btn-util-summarize": () => triggerAssistAction("summarize"),
    "btn-util-grammar": () => triggerAssistAction("grammar"),
    "btn-util-translate": () => {
      const lang = document.getElementById("util-translate-lang").value;
      triggerAssistAction("translate", lang);
    },
    ...
    // Brand Kit View
    "btn-save-brand": () => saveBrandKitProfile(),
    "btn-remove-logo": (e) => {
      if (e) e.stopPropagation();
      if (appState.brandKit) {
        appState.brandKit.logoBase64 = "";
      }
      appState.logoIsAutoGenerated = true;
      updateLogoPreview(null);
    },
    // Settings View
    "btn-save-api-settings": () => saveApiSettings(),
    "btn-save-profile": () => saveProfileDetails(),
    "btn-delete-profile-account": () => deleteUserAccount(),
    // Plans & Limits View
    "btn-current-free-limit": (e) => handleFreeLimitTrigger(e),
    "btn-subscribe-edu": (e) => handleEduSubscriptionTrigger(e),
    "btn-contact-sales": (e) => handleBizContactTrigger(e),
    // Modals
    "btn-verify-send-code": () => sendVerificationOTP(),
    "btn-verify-confirm-code": () => confirmVerificationOTP(),
    ...
    "btn-show-privacy-terms": (e) => {
      if (e) e.preventDefault();
      document.getElementById("modal-privacy-terms").style.display = "flex";
    },
    // QR Generator view
    "btn-download-qr": () => triggerQRCodeDownload(),
    "btn-generate-qr": () => {
      const url = document.getElementById("qr-input-url").value.trim();
      generateQR(url);
    }
  };
  ...
  // Wire up sidebar elements dynamically
  ...
  const signinPassToggle = document.getElementById("signin-pass-toggle");
  if (signinPassToggle) {
    signinPassToggle.onclick = (e) => {
      if (e) e.stopPropagation();
      togglePasswordVisibility("signin-password", "signin-pass-toggle");
    };
  }

  const signupPassToggle = document.getElementById("signup-pass-toggle");
  if (signupPassToggle) {
    signupPassToggle.onclick = (e) => {
      if (e) e.stopPropagation();
      togglePasswordVisibility("signup-password", "signup-pass-toggle");
    };
  }

  const geminiKeyToggle = document.getElementById("gemini-key-toggle");
  if (geminiKeyToggle) {
    geminiKeyToggle.onclick = (e) => {
      if (e) e.stopPropagation();
      togglePasswordVisibility("settings-gemini-key", "gemini-key-toggle");
    };
  }
...
window.triggerSaveDocument = triggerSaveDocument;
window.triggerShareLink = triggerShareLink;
window.saveBrandKitProfile = saveBrandKitProfile;
window.saveApiSettings = saveApiSettings;
window.saveProfileDetails = saveProfileDetails;
window.deleteUserAccount = deleteUserAccount;
window.removeBrandLogo = removeBrandLogo;
window.triggerQRCodeDownload = triggerQRCodeDownload;
window.sendVerificationOTP = sendVerificationOTP;
window.confirmVerificationOTP = confirmVerificationOTP;
